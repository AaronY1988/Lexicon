...
- **Global hotkey** — `⌃⌘D` from anywhere brings up a frosted-glass search panel, Spotlight-style — and you can set your own shortcut in Settings.
...
- **Share as a card** — turn any entry into a beautiful word card and copy it to the clipboard or save it as a PNG (warm Reading Room style, full entry, Lexicon wordmark).
- **History & favorites** — every lookup is remembered; star words to keep them around. Clear recent lookups any time (favorites are kept).
- **Dictionary preferences** — a Settings window lists every installed Apple dictionary; pick exactly which ones to search and drag to reorder them.
- **Adjustable text size** — scale the definition body text (85–150%) from Settings.
- **Themes** — switch between the warm *Reading Room* and a vivid *Luminous Glass* theme in Settings.
- **Study mode** — add words with the graduation-cap button and review them with spaced repetition (SM-2, the algorithm behind Anki). Flashcards quiz both directions and show English + Chinese together.
...
- **Change the hotkey:** set it in Settings → Global shortcut (no code needed). The default lives in `KeyCombo.controlCommandD` in `HotKeyManager.swift`.
...
- [x] Settings window — dictionary selection and text size *(done in 1.3.0)*
- [x] Spaced-repetition study mode *(done in 1.3.0)*
- [x] Customizable global hotkey in Settings *(done in 1.4.0)*
- [ ] Recent ⇄ Favorites toggle in the results footer
- [ ] Wikipedia rich preview
- [ ] Export favorites / study deck to Anki/CSV
- [ ] Inline thesaurus suggestions

## Changelog

Current version: **1.5.0**. See [CHANGELOG.md](CHANGELOG.md) for the full history.
